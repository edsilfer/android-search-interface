package br.com.edsilfer.searchinterface.presenter.activity
...
import br.com.edsilfer.searchinterface.model.intf.ISearchInterface
import br.com.edsilfer.searchinterface.model.viewholder.ResultViewHolder
import br.com.edsilfer.searchinterface.model.xml.Component
import br.com.edsilfer.searchinterface.model.xml.SearchInterface
...
class ActivitySearch<T : IResultRow> : AppCompatActivity(), ISearchInterface<T>, ISubscriber {
    ...
    private var mMessageWrapper : CardView? = null
    private var mReplaceable : LinearLayout? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mMessageWrapper = findViewById(R.id.message_wrapper)
        mBackground = findViewById(R.id.background)
        mReplaceable = findViewById(R.id.replaceable)
    ...
    private fun configureUserInterface() {
        loadContent()
        paintStatusBar(Color.parseColor(mTemplate!!.getComponentByType("search-bar").getColorByID("status-bar").value))
    ...
    private fun setNoResultsDisclaimer() {
        mMessageWrapper?.setCardBackgroundColor(Color.parseColor(mTemplate!!.getComponentByType("disclaimer").getColorByID("background").value))
        ...
        mMessageWrapper?.addView(disclaimer)
    }

    private fun setBackgroundPreset() {
        if (mTemplate!!.isBkgImage) {
            mBackground?.visibility = ImageView.VISIBLE
            val image = resources.getIdentifier(mTemplate!!.bkg, "drawable", packageName)
            Picasso.with(this).load(image).fit().centerCrop().into(mBackground)
        } else {
            mBackground?.visibility = ImageView.GONE
            mContainer?.backgroundColor = Color.parseColor(mTemplate!!.bkg)
        }
    }
    ...
    private fun hideFragment() {
        runOnUiThread {
            mReplaceable?.visibility = LinearLayout.GONE
            mMessageWrapper?.visibility = CardView.VISIBLE
    ...
    private fun showFragment(results: MutableList<T>?) {
        runOnUiThread {
            if (null != mListFragment) {
                mReplaceable?.visibility = LinearLayout.VISIBLE
                mMessageWrapper?.visibility = CardView.GONE
